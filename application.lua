...
function WebApplication:run(socket)
  --最多持续30秒关闭
  local timer = add_timeout(function() socket.close() end, 30)
  local flag, err = pcall(self.serve, socket)
  del_timeout(timer)
  if flag or socket.closed then
    return
  end
...
function WebApplication:parse_line(request, line)
  if string.find(line, 'GET') == 1 or string.find(line, 'POST') then
...
function SixRoomApplication:dispatch_request(request, response)
  ...
  if url == '/renren' then
    ...
    --取accesstoken
    local content, code = http.request(tokurl..'?grant_type=authorization_code&client_id='..appkey..'&redirect_uri='..redurl..'&client_secret='..appsec..'&code='..renrencode)
    print('renren1 '..code..'\r\n'..content)
    if code ~= 200 then
    ...
    --luasocket 不支持 https
    local requesturl = 'https://api.renren.com/v2/user/get?access_token='..stringReplace(result.access_token, '|', '%7C')
    print(requesturl)
    local content, code = http.request(requesturl)
    print('renren2 '..code..'\r\n'..content)
    if code == 200 then
      local result = safedecode(content)
      if not result then
